 ...
 *   $mailbox = new Imap($host, $user, $pass, $port, $encryption, $folder);
...
class Imap {
	 ...
	 * @param $port (int)
	 *   Example: 933
	 * @param $encryption (bool)
	 *   Valid values are ssl, tls or none.
	 * @param $folder (string)
	 *   IMAP Folder to open.
	 * @param $user (string)
	...
	public function __construct($host, $user, $pass, $port, $encryption = 'ssl', $folder = 'INBOX') {
		...
		$this->changeLoginInfo($host, $user, $pass, $port, $encryption, $folder);
	}
	...
	public function changeFolder($folderName) {
		switch($this->encryption) {
			case 'tls':
			case 'ssl':
				$address = '{' . $this->host . ':' . $this->port . '/imap/'.$this->encryption.'}' . $folderName;
			break;
	 ...
	 */
	public function changeLoginInfo($host, $user, $pass, $port, $encryption, $folder) {
		switch($encryption) {
			case 'tls':
			case 'ssl':
				$baseAddress = '{' . $host . ':' . $port . '/imap/'.$encryption.'}';
